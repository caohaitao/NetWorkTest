...
TEST(test_net, section2)
{
        ...
        Net * net = new Net(layer_datas);
        //for (int i = 0; i < net->GetFirstLayer()->GetNextLayer()->GetNodeSize();i++) {
        //        Node * n = net->GetFirstLayer()->GetNextLayer()->GetNode(i);
        //        for (int j = 0; j < n->GetInputSize();j++) {
        //                n->SetParam(j, n->GetParam(j));
        //        }
        //}
        for (int k = 0; k < 10000; k++) {
                for (int i = 0; i < 4; i++) {
